...
plt.errorbar(incl_data, np.abs(fwhm_data), yerr=err_fwhm, markerfacecolor='#bdbdbd', markeredgecolor='None', linestyle='None', marker='o', capsize=3, label='$NC$')
for i in range(len(ID)):
    plt.annotate(ID[i], (incl_data[i]+0.3, np.abs(fwhm_data[i])+0.3))
plt.errorbar(incl_data, np.abs(fwhm_data1), yerr=err_fwhm1, markerfacecolor='#252525', markeredgecolor='None', linestyle='None', marker='*', capsize=3, label='$BC$')
for i in range(len(ID)):
