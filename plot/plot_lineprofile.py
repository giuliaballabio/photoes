...
## CREATE A MULTIPLOT WITH LINES AT DIFFERENT INCLINATIONS VARYING THE OUTER RADIUS

# b = 1.00
# incl_deg = [0.0, 20.0, 45.0, 90.0]
# r_in = 0.1
# r_out = [0.9, 1.0, 1.5] #9.5
# cs = 10
# species = 'NeII'
#
# path_file = []
...
# for row in range(2):
#     for col in range(2):
#         ax[row,col].plot(v1[2*row+col], line_flux1[2*row+col] / np.amax(line_flux1[2*row+col]), color='#fecc5c', label='$R_{out}='+str(r_out[0])+'$')
#         ax[row,col].plot(v2[2*row+col], line_flux2[2*row+col] / np.amax(line_flux2[2*row+col]), color='#fd8d3c', label='$R_{out}='+str(r_out[1])+'$')
#         ax[row,col].plot(v3[2*row+col], line_flux3[2*row+col] / np.amax(line_flux3[2*row+col]), color='#f03b20', label='$R_{out}='+str(r_out[2])+'$')
#         # ax[row,col].plot(v4[2*row+col], line_flux4[2*row+col] / np.amax(line_flux4[2*row+col]), color='#bd0026', label='$R_{out}='+str(r_out[3])+'$')
#         ax[row,col].plot(v_hydro[2*row+col], line_flux_hydro[2*row+col] / np.amax(line_flux_hydro[2*row+col]), color='#2171b5', label='$hydro$') #, label='$i='+str(incl_deg[2*row+col])+'$')
#         leg1 = ax[0,0].legend(loc='upper right')
#         leg2 = ax[row,col].legend(['$i='+str(incl_deg[2*row+col])+'$'], loc='upper left', frameon=False, handlelength=0, handletextpad=0)
#         for item in leg2.legendHandles:
