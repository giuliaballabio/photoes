...
plt.errorbar(incl_data2, fwhm_data2/np.abs(vpeak_data2), yerr=(err_fwhm2/err_vpeak2), color='k', linestyle='None', marker='*', capsize=3, label='$Pascucci\,&\,Sterzik\,(2009)$')
for i in range(len(name)):
    plt.annotate(name[i], (incl_data2[i]+0.3, np.abs(fwhm_data2[i]/vpeak_data2[i])+0.3))
plt.errorbar(incl_data3, fwhm_data3/np.abs(vpeak_data3), yerr=(err_fwhm3/err_vpeak3), color='k', linestyle='None', marker='d', capsize=3, label='$Baldovin-Saavedra\,(2012)$')
for i in range(len(name3)):
    plt.annotate(name3[i], (incl_data3[i]-12.0, np.abs(fwhm_data3[i]/vpeak_data3[i])-0.1))
plt.errorbar(incl_data, np.abs(fwhm_data/np.abs(vpeak_data)), yerr=(err_fwhm/err_vpeak), color='k', linestyle='None', marker='o', capsize=3, label='$Sacco\,et\,al.\,(2012)$')
for i in range(len(ID)):
    plt.annotate(ID[i], (incl_data[i]+0.3, np.abs(fwhm_data[i]/vpeak_data[i])+0.3))
plt.xlabel(r'$i \, [^{\circ}]$')
...
plt.figure()
plt.plot(incl_deg, fwhm1/v_centr1, color='#c6dbef', linestyle='-', linewidth=2.5, marker='None', markeredgecolor='#c6dbef', label='$c_{s} = 3 \, km/s$')
plt.plot(incl_deg, fwhm2/v_centr2, color='#2171b5', linestyle='-', linewidth=2.5, marker='None', markeredgecolor='#2171b5', label='$c_{s} = 5 \, km/s$')
plt.plot(incl_deg, fwhm3/v_centr3, color='#08306b', linestyle='-', linewidth=2.5, marker='None', markeredgecolor='#08306b', label='$c_{s} = 10 \, km/s$')
plt.errorbar(incl_data, fwhm_data/vcentr_data, yerr=(err_fwhm/err_vcentr), color='k', markeredgecolor='None', linestyle='None', marker='o', capsize=3, label='$NC$')
for i in range(len(ID)):
    plt.annotate(ID[i], (incl_data[i]+0.5, np.abs(fwhm_data[i]/vcentr_data[i])+0.5), color='k')
plt.errorbar(incl_sx, fwhm_sx/vcentr_sx, yerr=(err_fwhm_sx/err_vcentr_sx), color='k', markeredgecolor='None', linestyle='None', marker='o', capsize=3)
for i in range(len(ID_sx)):
    plt.annotate(ID_sx[i], (incl_sx[i]-10.0, np.abs(fwhm_sx[i]/vcentr_sx[i])+0.5), color='k')
plt.errorbar(incl_down, fwhm_down/vcentr_down, yerr=(err_fwhm_down/err_vcentr_down), color='k', markeredgecolor='None', linestyle='None', marker='o', capsize=3)
for i in range(len(ID_down)):
    plt.annotate(ID_down[i], (incl_down[i]+0.5, np.abs(fwhm_down[i]/vcentr_down[i])-1.2), color='k')
plt.xlabel(r'$i \, [^{\circ}]$')
plt.ylabel(r'$FWHM \, [km/s]$')
plt.xticks(np.arange(min(incl_deg), max(incl_deg)+10., 10.0))
plt.title('$[OI] \, 6300 \AA$')
