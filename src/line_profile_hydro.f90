	...
	!                Input file from hydro simulations.
	!
	...
	double precision, dimension(1:n_r,1:n_theta)     :: dV, C, cell_flux
	double precision, dimension(1:n_r,1:n_theta)     :: v_los
	...
	open (unit=1, file = '../data/grid_r.dat')
	do i=1, n_r
	    read (1,*) r(i)
	enddo
	close(1)
	...
	!! GET THE DATA FROM THE FILES AND CREATE 2D ARRAYS !!
	print *, 'Reading data from files...'

	open (unit=2, file='../data/rho_mean.dat')
	open (unit=3, file='../data/v_r_mean.dat')
	open (unit=4, file='../data/v_th_mean.dat')
	open (unit=5, file='../data/v_phi_mean.dat')
	do i=1, n_r
	    do j=1, n_theta0
	        read (2,*) rho2d(i,j)
	        read (3,*) v_r2d(i,j)
	...
	if (.not.init) then
	    open(unit=8, file='bound_cond.txt')
	else
	    open(unit=8, file='bound_cond.txt', status='old', position='append')
	endif
	do i=1, n_r
	    write(8,'(4(es18.10,1X))') r(i), rho(i,249), v_theta(i,249), v_phi(i,250)
	enddo
	close(8)
	...
	do i=1, n_r
	    do j=1, n_theta
	        dV(i,j) = r(i)*r(i) * sinth(j) * dr(i) * dtheta * dphi
	    ...
	    enddo
	enddo
	tot_flux = sum(cell_flux)
	print *, '-----------------------------------------------------------'
	print *, '   Total flux=', tot_flux, 'Lsun'
	print *, '-----------------------------------------------------------'
	...
	if (.not.init) then
	    ...
	    open(unit=7, file='output.txt', status='old', position='append')
	endif
	do l=1, n_v
	    write(7,'(2(es18.10,1X))') v(l), line_flux(l)
	enddo
	close(7)
