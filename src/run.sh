...
for ((i=0;i<${#array_b[@]};++i)); do
  # echo "(${array_b[i]}, ${array_ub[i]})"
	for r_inner in 0.1; do
		for r_outer in 5.0 5.5 6.0 6.5 7.0 7.5 8.0 8.5 9.0; do
			for incl in 90.0; do
				...
				if [ ! -d $RUNDIR/../cs$string_cs\kms/$species/mdot10e-8/data_b${array_b[i]}\_r$r_inner\_r$r_outer/incl_$incl ]; then
					if [ ! -d $RUNDIR/../cs$string_cs\kms/$species/mdot10e-8/data_b${array_b[i]}\_r$r_inner\_r$r_outer ]; then
						if [ ! -d $RUNDIR/../cs$string_cs\kms/$species/mdot10e-8 ]; then
							...
							mkdir $RUNDIR/../cs$string_cs\kms/$species/mdot10e-8
						fi
						mkdir $RUNDIR/../cs$string_cs\kms/$species/mdot10e-8/data_b${array_b[i]}\_r$r_inner\_r$r_outer
					fi
					mkdir $RUNDIR/../cs$string_cs\kms/$species/mdot10e-8/data_b${array_b[i]}\_r$r_inner\_r$r_outer/incl_$incl
				fi



				# Copy files to this new directory
				cp selfsimilar_solutions photoes submit-job* $RUNDIR/../cs$string_cs\kms/$species/mdot10e-8/data_b${array_b[i]}\_r$r_inner\_r$r_outer/incl_$incl

				# Remove unuseful files
				rm *genmod* photoes selfsimilar_solutions

				# Submit the job on dial or alice
				cd $RUNDIR/../cs$string_cs\kms/$species/mdot10e-8/data_b${array_b[i]}\_r$r_inner\_r$r_outer/incl_$incl
