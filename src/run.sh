			...
			for incl in 27.0; do #35.0 45.0 50.0 60.0 68.0 75.0 82.0; do
				...
				if [ ! -d $RUNDIR/../cs$string_cs\kms/$species/data_b${array_b[i]}\_r$r_inner\_r$r_outer/incl_$incl ]; then
					if [ ! -d $RUNDIR/../cs$string_cs\kms/$species/data_b${array_b[i]}\_r$r_inner\_r$r_outer ]; then
						...
						mkdir $RUNDIR/../cs$string_cs\kms/$species/data_b${array_b[i]}\_r$r_inner\_r$r_outer
					fi
					mkdir $RUNDIR/../cs$string_cs\kms/$species/data_b${array_b[i]}\_r$r_inner\_r$r_outer/incl_$incl
				fi



				# Copy files to this new directory
				cp selfsimilar_solutions photoes submit-job* $RUNDIR/../cs$string_cs\kms/$species/data_b${array_b[i]}\_r$r_inner\_r$r_outer/incl_$incl

				# Remove unuseful files
				rm *genmod* photoes selfsimilar_solutions

				# Submit the job on dial or alice
				cd $RUNDIR/../cs$string_cs\kms/$species/data_b${array_b[i]}\_r$r_inner\_r$r_outer/incl_$incl
				rm photoes_b*
